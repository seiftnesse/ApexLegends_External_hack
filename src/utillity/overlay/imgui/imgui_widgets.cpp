...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	RenderFrame(bb.Min, bb.Max, ImColor(29, 29, 29, 255), true, style.FrameRounding);
	window->DrawList->AddRect(bb.Min, bb.Max, hovered ? ImColor(251, 160, 227, 255) : (ImColor(0.35f, 0.35f, 0.35f, 100 / 255.f)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	ImU32 text_col = GetColorU32(ImVec4(1.f, 1.f, 1.f, 0.5f));

	const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
	window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), ImColor(29, 29, 29, 255), 2.f);

	window->DrawList->AddRect(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), hovered ? ImColor(251, 160, 227, 255) : ImColor(0.35f, 0.35f, 0.35f, 100 / 255.f), 2.f);

	if (*v) {
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(5, 5), check_bb.Max - ImVec2(5, 5), ImColor(251, 160, 227, 255), 2.f);
		text_col = GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f));
	}

	PushStyleColor(ImGuiCol_Text, text_col);
	RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y + 1.5f), label);
	PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 15), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f + 15));
	...
	if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), ImColor(29, 29, 29, 225), style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);

	if (!(flags & ImGuiComboFlags_NoArrowButton))
	{
		ImU32 bg_col = ImColor(29, 29, 29, 255);
		ImU32 text_col = GetColorU32(ImGuiCol_Text);
		window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, bg_col, style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
		if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
			RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, popup_open ? ImGuiDir_Up : ImGuiDir_Down, 1.0f);
	}
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, hovered ? ImColor(251, 160, 227, 255) : ImColor(0.35f, 0.35f, 0.35f, 100 / 255.f), 2.f, 0, 0.f);
	...
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - style.FramePadding.y - label_size.y), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(29, 29, 29, 255), true, 2.5f);
	...
	if (grab_bb.Max.x > grab_bb.Min.x)
		window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Max + ImVec2(2, 2), ImColor(251, 160, 227, 255), 2.f);

	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, hovered ? ImColor(251, 160, 227, 255) : ImColor(0.35f, 0.35f, 0.35f, 100 / 255.f), 2.f);
